...
{
    // All source code in this class refer to the author
    /*
     * Copyright by Tolga Birdal
     * It is on the CodeProject website
     */
    // Thank you Tolga Birdal
    ...
    class OtsuThreshold
    {
        ...
        public int getOtsuThresholdFromHistogram(int[] hist)
        {
            int length = hist.GetLength(0);
            byte t = 0;
            float[] vet = new float[length];
            ...
            {
                p1 = Px(0, k, hist);
                p2 = Px(k + 1, length-1, hist);
                p12 = p1 * p2;
                if (p12 == 0)
                    p12 = 1;
                float diff = (Mx(0, k, hist) * p2) - (Mx(k + 1, length-1, hist) * p1);
                vet[k] = (float)diff * diff / p12;
            }

            t = (byte)findMax(vet, length);
